...
static constexpr uint32_t R = static_cast<uint32_t>((1ULL << (2 * Q_BIT_WIDTH)) / Q);
...
public:
  ...
  {
    const auto n64 = static_cast<uint64_t>(n);
    zq_t base = *this;

    zq_t res = (static_cast<bool>(n64 & 0b1UL)) ? base : zq_t(1);

    const size_t zeros = static_cast<size_t>(std::countl_zero(n64));
    const size_t till = 64UL - zeros;

    for (size_t i = 1; i < till; i++) {
      base = base * base;

      res = res * ((static_cast<bool>((n64 >> i) & 0b1UL)) ? base : zq_t(1));
    }
