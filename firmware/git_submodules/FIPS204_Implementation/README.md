...
Bachelor thesis by Julien Wyss and Levin Hofer <br>
06. June 2026

This repository integrates FIPS 204 (ML-DSA) with a classical signature baseline (ECDSA), including a firmware-level test and performance evaluation setup.

---

## Table of Contents

- [FIPS204\_Implementation](#fips204_implementation)
...
This repository provides a test structure to evaluate and compare the post-quantum and classical signature algorithms FIPS 204 (ML-DSA) and ECDSA. During testing, the firmware performs full functional operations (key generation, signing, and verification). It then transmits the public key, signature, and original message to the host for independent validation.

At runtime, this program provides two test modes, selectable via onboard switches: a manual single-run mode triggered by a pushbutton, and an automatic multi-run benchmarking workflow. One switch selects the algorithm (post-quantum vs. classical), while the other toggles between manual and automatic operation. Results, public keys, signatures, and payloads are streamed to the host in labeled, chunked blocks for independent validation and further processing.

### What This Repository Measures

This repository captures per-operation execution timings (key generation, signing, and verification) recorded per run and summarized as statistical aggregates (mean, min, and max).

- **Time Measurement:** To measure the execution time required by each algorithm, a software-based timing test is implemented on the firmware side. The firmware records cycle counts around each cryptographic operation using Zephyr timing functions and converts them to microseconds for reproducible comparison across multiple test runs. The goal is to obtain stable results, unaffected by transient CPU load, by calculating the mean execution time of each task.
  
- **Memory Usage:** Using a similar approach, a software-based test is implemented to estimate the peak memory usage of the executed functions. The firmware samples the stack and heap high-water mark before and after each operation, which provides an approximation of the memory footprint during execution and allows for comparison between the PQC algorithms and their classical counterparts.

For power analysis, the firmware toggles a dedicated GPIO probe immediately before and after each timed operation so that external instruments can capture current/power traces aligned with the operation window. Time summaries, stack measurements, and payloads are streamed as structured CSV blocks for host collection and post-processing.

---

## Affiliation

This repository was developed as one of three independent, algorithm-specific sub-repositories. Each provides an embedded test harness to evaluate and compare post-quantum and classical signature or key-exchange algorithms on a Zephyr STM32 target.
...
- [FIPS204_Implementation](https://github.zhaw.ch/PQC-on-Microcontrollers/FIPS204_Implementation.git) (This Repo)
- [FIPS205_Implementation](https://github.zhaw.ch/PQC-on-Microcontrollers/FIPS205_Implementation.git)

To ensure modularity, the cryptographic implementations (like this one) are integrated behind a Zephyr interface. The algorithms are organized into pairs, where each pair contains one classical algorithm and one post-quantum counterpart for direct comparison. The differing implementation pairs are selected at build time through `west build` command options, allowing the Zephyr project to be compiled with alternative pairs without modifying the repository structure or logic.

Therefore, this repository is not standalone and requires its upstream Zephyr project repository:  
[PQC_BA_Zephyrproject](https://github.zhaw.ch/PQC-on-Microcontrollers/PQC_BA_Zephyrproject.git)

The task of this repository is to provide an implementation base for the two algorithms (FIPS 204 and ECDSA), as well as an embedded test harness to evaluate and compare them.

Additionally, the Host-side Python listener application framework is required to view, evaluate, and process the data sent by the board over USB. It acts as both a debugging console and a communication interface for the board.  
...
- **Algorithm Implementations:** FIPS 204 (ML-DSA) and ECDSA integrated for an embedded target.
...
- `prj.conf` — Main Zephyr configuration (stack size, mbedTLS/ECDSA options, USB/console/logging settings, enabling peripherals).
- `mc1board.overlay` — Device-tree overlay for USB CDC-ACM console routing on the target board.
- `README.md` — Project documentation.
- `src/main.cpp` — Main firmware flow. Initializes hardware, generates keys, signs a test message, and sends the payload over USB.
- `src/ECDSA_implementation.cpp` & `.h` — mbedTLS-based ECDSA implementation.
- `src/FiPS204_implementation.cpp` & `.h` — Wrapper implementation for ML-DSA key generation, signing, and verification.
- `src/USBcommunication.cpp` & `.h` — USB output helpers used to print logs and send payloads to the host.
- `Fips204 C++ implementation Full Repo/` — Upstream source directory containing the ML-DSA (FIPS 204) reference implementation and its dependencies (`ml-dsa-master`, `RandomShake-main`, and `sha3-master`).
...
2. **Build the Project:** Use the West build tool to build and flash this program from the upstream Zephyr project. See [Affiliation](#affiliation) for more information. This project cannot be used as a standalone application.
   ...
   ```bash
   # Build
   west build -b mc1board firmware/git_submodules/FIPS204_Implementation/ -p always
   ...
   ```
3. **Reconnect USB:** If only one USB cable is available, you must reconnect the board using the communication USB port (instead of the J-Link OB Port) after a successful flash.

![FIPS204 MC1Board](pictures/Picture_MC1_board_usbConnections.jpg)
...
5. **Choose Algorithm and Test Method:** Once connected, the Host framework displays the received information from the board. 
   ...
   - Start or rerun tests manually with button `T3` (When `LED0` is blinking, the program is ready for user input).

![FIPS204 TestRun console output Screenshot](pictures/Screenshot_Fips204_testRun_consoleOutput.png)
...
![FIPS204 BatchTestRun console output Screenshot](pictures/Messresultate_MeanTime_Board_FIPS204_1.png)
...
- **Buttons / Pushbuttons**
  - `T3` (`t3` in the devicetree): Manual test trigger. <br> Pressing it requests a test run.
- **Switches**
  - `SW0` (`sw0` in the devicetree): Algorithm selection switch.<br> High = FIPS 204 (ML-DSA), Low = ECDSA.
  - `SW1` (`sw1` in the devicetree): Mode selection switch.<br> High = automatic multi-run tests, Low = manual trigger via `T3`.
- **LEDs**
  - `LED0` (`led0` in the devicetree): Status/heartbeat indicator.<br> Fast blinking means the program is waiting for user input.
...
FIPS204,key_generation,1,12345,1024,2048,1024,true
FIPS204,signing,1,54321,2048,3072,1024,true
FIPS204,verify,1,6789,2048,2500,452,true
#stop#
```
...
- The `algorithm` column is either `ECDSA` or `FIPS204`.
...
**Credit:**  
This project integrates a post quantum ML DSA implementation (FIPS 204) via a local wrapper around the upstream header only itzmeanjan's [ml-dsa](https://github.com/itzmeanjan/ml-dsa.git) Git repository and the firmware targets the ml\_dsa\_65 parameter set. The ML DSA code depends on SHA 3 / XOF primitives and an XOF based CSPRNG; those upstream components (itzmeanjan's [sha3](https://github.com/itzmeanjan/sha3.git) and [RandomShake](https://github.com/itzmeanjan/RandomShake.git) Git repositories) are present in the repository subtree and consumed by the ML DSA implementation. The classical baseline uses ECDSA on curve secp256r1 (P 256) via the platform crypto stack (mbedTLS) for RNG, key generation, signing and verification, with messages hashed using SHA 256 before signing/verification. ML DSA seeds and signing randomness are filled from Zephyr entropy (sys\_rand\_get()), while ECDSA uses mbedTLS CTR DRBG + entropy. 
